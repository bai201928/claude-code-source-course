...
def create_runtime_context(
    *,
    ...
    return RuntimeContext(
        runtime_id, configuration_revision, model_adapter, resolved_started_at
    )
...
class SessionStateStore:
    def __init__(
        ...
        self._store = SynchronousStore(_create_session_state(1, initial_values), observer)
...
def read_fresh_session(
    ...
    return FreshSessionRead(request.request_id, current.revision, current.values)
...
def _create_session_state(
    ...
    return SessionState(revision, _freeze_mapping(values))
