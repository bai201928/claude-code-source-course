...
class ConversationStoreTests(unittest.TestCase):
    def test_one_conversation_accepts_only_one_runtime_owner(self) -> None:
        store = ConversationStore()
        owner = object()
        store.bind_runtime(owner)
        store.bind_runtime(owner)
        with self.assertRaisesRegex(ConversationOwnershipError, "already bound"):
            store.bind_runtime(object())

    def test_active_run_lease_blocks_external_writers(self) -> None:
        store = ConversationStore()
        owner = object()
        store.bind_runtime(owner)
        ...
        store.append(1, (human("after", "allowed"),))
