...
class H0RuntimeCore:
    def __init__(self, create_harness: Callable[[], H0Harness]) -> None:
        self._create_harness = create_harness

    async def run(self, command: RuntimeCommand) -> AsyncIterator[DomainEvent]:
        harness = self._create_harness()
        try:
            async for message in harness.run(command["text"]):
                if message["kind"] == "progress":
                    yield {"type": "progress", "text": message["text"]}
                elif message["kind"] == "assistant":
                    yield {"type": "result", "text": message["text"]}
        except Exception as error:
            yield {"type": "failure", "message": str(error)}
